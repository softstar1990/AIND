    ...
    def select(self):
        ...
        for n_components in range(self.min_n_components, self.max_n_components+1):
            try:
                ...
                p = n_components * (n_components + d * 2) 
...
class SelectorDIC(ModelSelector):
    ...
    def select(self):
        ...
        if len(all_n_components) > 1:    
            for score in all_scores:
                dic = score - (sum_logL-score) / (len(all_n_components)-1)
                all_dic.append(dic)
            best_num_components = all_n_components[np.argmax(all_dic)]
        elif len(all_n_components) == 1:
        ...
        for n_components in range(self.min_n_components, self.max_n_components+1):
            try:
                if len(self.sequences) > 2:
                    scores = [] #store scores for each split
